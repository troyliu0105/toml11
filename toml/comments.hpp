...
#  define TOML11_GCC48X_WORKAROUND_RETURN_TYPE iterator
    ...
    iterator insert(TOML11_GCC48X_WORKAROUND_ITERATOR p, const std::string& x)
    {
        return comments.insert(p, x);
    }
    iterator insert(TOML11_GCC48X_WORKAROUND_ITERATOR p, std::string&&      x)
    {
        return comments.insert(p, std::move(x));
    }
    TOML11_GCC48X_WORKAROUND_RETURN_TYPE
    insert(TOML11_GCC48X_WORKAROUND_ITERATOR p,
           size_type n, const std::string& x)
    {
    ...
    TOML11_GCC48X_WORKAROUND_RETURN_TYPE
    insert(TOML11_GCC48X_WORKAROUND_ITERATOR p,
           std::initializer_list<std::string> ini)
    {
        return comments.insert(p, ini);
    }

    template<typename ... Ts>
    iterator emplace(TOML11_GCC48X_WORKAROUND_ITERATOR p, Ts&& ... args)
    {
        return comments.emplace(p, std::forward<Ts>(args)...);
    }

    iterator erase(TOML11_GCC48X_WORKAROUND_ITERATOR pos)
    {
        return comments.erase(pos);
    }
